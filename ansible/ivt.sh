...
check_system() {
  ...
  if [ ${PROCESSOR} -lt 2817 -a ${PROCESSOR} -ne 1090 ]; then
    echo "ERROR: A processor level (${PROCESSOR}) lower than IBM zEnterprise EC12 has been detected. The minimum requirement to compile and execute z/OS Open Tools is IBM zEnterprise EC12."
    exit 1
  fi
  echo "Success"
