#!/bin/bash
#
# Run UpdateDocs function locally in the current repository
# This skips the git clone step and runs directly in the current directory
#
set -e

echo "Running UpdateDocs locally in current repository..."
...
echo "Checking for .md extensions in links..."
if grep 'href="\./[^"]*"' docs/reference/*.md 2>/dev/null | grep -v '\.md"'; then
  echo "ERROR: Links without .md extension found!"
  exit 1
else
  echo "✓ All links have .md extension"
fi

echo ""
echo "SUCCESS: All documentation generated correctly!"
echo ""
