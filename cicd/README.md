...
* **Key Parameters**:
  ...
  * `SOURCE_DIR`: Optional directory containing source files (relative to project root). **Default**: automatically uses `SOURCES` directory if it exists. Set to empty or any other directory name to override. Example: `SOURCES`, `rpm-sources`. Leave empty for virtual/meta packages.
