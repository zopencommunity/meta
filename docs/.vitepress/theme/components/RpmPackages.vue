...
import catalogData from "../../../api/rpm_packages.json";
...
type SortKey = "name" | "version" | "verification" | "recent";
type SortDirection = "asc" | "desc";
const sort = ref<SortKey>("name");
const sortDirection = ref<SortDirection>("asc");
const copied = ref("");

const dnfSetup = `# Add to dnf configuration
cat >> /opt/pkg/etc/dnf/dnf.conf <<EOF
[main]
reposdir=/opt/pkg/etc/yum.repos.d
EOF

# Configure zopen repository
...
function verificationKind(pkg: RpmPackage) {
...
const filteredPackages = computed(() => {
  ...
  return packages.sort((left, right) => {
    ...
    if (sort.value === "recent") {
      comparison = (left.buildTime || "").localeCompare(right.buildTime || "");
...
function installCommand(pkg: RpmPackage) {
  return `dnf5 install ${pkg.name}`;
}
...
<template>
  <div class="rpm-packages">
    <section class="catalog-intro">
      <div>
        <p class="eyebrow">Available now</p>
        <h1>RPM packages for z/OS</h1>
        <p class="intro-copy">
          Browse RPM packages published by the zopen community, see version information,
          and review the latest reported port-verification results before installing with DNF5.
        </p>
      </div>
      <div class="catalog-summary" aria-label="Catalogue summary">
        <div><strong>{{ catalog.packageCount }}</strong><span>packages</span></div>
        <div><strong>{{ catalog.rpmCount }}</strong><span>RPMs</span></div>
        <div><strong>s390x</strong><span>architecture</span></div>
      </div>
    </section>
    ...
    <details class="setup-panel">
      <summary>Configure DNF5 for the zopen repository</summary>
      <div class="setup-content">
        <div class="command-heading"><strong>DNF5 Configuration</strong><button type="button" @click="copyText(dnfSetup, 'dnf-setup')">{{ copied === "dnf-setup" ? "Copied" : "Copy setup" }}</button></div>
        <pre><code>{{ dnfSetup }}</code></pre>
      </div>
      <p>See the <a :href="withBase('/Guides/RpmSetup')">installation guide</a> for complete setup instructions.</p>
    </details>
    ...
    <div class="results-heading">
      ...
      <span>{{ sort === "recent" ? `${sortDirection === "desc" ? "Newest" : "Oldest"} builds first` : `Catalogue refreshed ${displayDate(catalog.generatedAt)}` }}</span>
    </div>

    <div v-if="filteredPackages.length" class="package-table" role="table" aria-label="Available RPM packages">
      <div class="package-header" role="row">
        ...
        <span role="columnheader" :aria-sort="ariaSort('recent')"><button type="button" @click="changeSort('recent')">Build date <span aria-hidden="true">{{ sortIndicator('recent') }}</span></button></span>
      </div>
      ...
      <details v-for="pkg in filteredPackages" :key="pkg.name" class="package-row">
        <summary>
          <span class="package-identity">
            ...
            <small>{{ pkg.summary || "RPM package published for z/OS" }}</small>
          </span>
          <span class="version-value" data-label="Version">{{ pkg.version }}-{{ pkg.release }}</span>
          ...
          <span class="published-value" data-label="Build date">{{ displayDate(pkg.buildTime) }}</span>
        </summary>
        ...
        <div class="package-details">
          <div class="detail-metadata">
            ...
            <div><span>Architecture</span><strong>{{ pkg.architectures.join(", ") }}</strong></div>
          </div>
          <div class="install-actions">
            <button type="button" @click="copyText(installCommand(pkg), `${pkg.name}-install`)">{{ copied === `${pkg.name}-install` ? "Copied" : `Copy install command` }}</button>
          </div>
          <div class="detail-links">
            <a :href="pkg.rpmUrl" target="_blank" rel="noreferrer">Download RPM</a>
...
<style scoped>
.rpm-packages { color: var(--vp-c-text-1); }
...
.setup-panel pre { margin: 0 0 14px; padding: 12px; overflow-x: auto; border-radius: 8px; background: var(--vp-code-block-bg); font-size: 0.78rem; }
.catalog-controls { display: grid; grid-template-columns: minmax(240px, 1.6fr) repeat(2, minmax(150px, 0.7fr)); gap: 12px; padding: 16px; border: 1px solid var(--vp-c-divider); border-radius: 12px; background: var(--vp-c-bg-soft); }
...
.package-header, .package-row > summary { display: grid; grid-template-columns: minmax(230px, 2fr) minmax(120px, 0.8fr) minmax(175px, 1fr) minmax(110px, 0.75fr); gap: 16px; align-items: center; }
...
.package-identity small, .verification-value small { color: var(--vp-c-text-3); font-size: 0.72rem; font-weight: 450; line-height: 1.35; }
