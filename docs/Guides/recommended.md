...
This guide provides a recommended set of open source tools for various roles within the zopen community. Each role — from system administrators to AI engineers — has unique needs, and these tools aim to boost productivity, enhance security, and simplify workflows on the mainframe. All tools can be installed, updated, and managed via the [zopen package manager](/Guides/ThePackageManager).
...
- `openssh` — Secure remote login and file transfer (includes `sftp`, `ssh`, and `ssh-copy-id`).
...
- `c3270` — Terminal emulator to access ISPF/TSO via z/OS UNIX.
- `bash` — Shell scripting and command interpreter.
- `logrotate` — Automatic rotation and compression of logs.
- `tmux` and `screen` — Terminal multiplexers for managing sessions.
- `tree` — Directory and file structure visualization.
- `rsync` *(work in progress)* — File synchronization and transfer.
...
- `ncdu` — Disk usage analyzer to detect files and directories using up disk space.
- `zip`, `unzip`, `gzip`, `tar`, `bzip2`, `xz`, `zstd` — Compression and archiving tools.
...
- `coreutils` — Essential core utilities for daily operations.
- `util-linux` — Common utilities (e.g., `whereis`).
- `moreutils` — Additional utilities such as `sponge`, `parallel`, and more.
- `findutils`, `grep`, `sed`, `diff` — GNU-based utilities with additional function over `/bin` tools.
...
- `ninja` — Very fast build system for compiling projects.
- `make` — Build system for compiling projects.
